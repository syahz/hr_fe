...
import { useGetFeedbackDetails } from '@/hooks/api/useFeedback'

interface DetailsFeedbackModalProps {
...
export function DetailsFeedbackModal({ feedbackId, isOpen, onClose }: DetailsFeedbackModalProps) {
  // useGetFeedbackDetails now accepts optional id and will only run when id is provided
  const { data: feedback, isLoading, error } = useGetFeedbackDetails(feedbackId ?? undefined)
    ...
    if (feedback) {
      return (
        <div className="space-y-6 pt-2 pb-4">
          ...
          {/* Bagian Unit */}
          <div>
            <h3 className="text-sm font-medium text-muted-foreground mb-2">Unit Terkait</h3>
            <p className="font-medium text-base text-foreground">{feedback.unit?.name ?? '-'}</p>
          </div>
