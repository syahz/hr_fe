...
import { getDashboardHeadOfficeSearchConfig } from '@/config/search-config'
import { DataTableSearch } from '@/components/table/DataTableSearch'
import React, { useMemo, useState } from 'react'
import { DataTablePagination } from '@/components/table/DataTablePagination'
import { useGetAdminDashboardFeedbacksHeadOffice } from '@/hooks/api/useFeedbackHO'
import { Card, CardContent, CardHeader, CardTitle } from '@/components/ui/card'
import { FileText } from 'lucide-react'
import { FeedbackHOTable } from '@/features/(protected)/admin/dashboardHeadOffice/components/table/FeedbackHOTable'
import { FeedbackHOColumns } from '@/features/(protected)/admin/dashboardHeadOffice/components/table/FeedbackHOColumns'
...
const Index = () => {
  ...
  const divisionOptions = useMemo(() => {
    ...
    return divisionsData.data.map((d) => ({ value: d.id, label: d.name }))
  }, [divisionsData])

  const dynamicSearchConfig = useMemo(() => getDashboardHeadOfficeSearchConfig(divisionOptions), [divisionOptions])
  ...
  const selectedDivisionName = useMemo(() => {
    ...
    const match = divisionOptions.find((d) => String(d.value) === String(selectedDivisionId))
  ...
  const { data: dashboardData } = useGetAdminDashboardFeedbacksHeadOffice(queryParams)
  console.log('Dashboard Data:', dashboardData)
  const columns = useMemo(() => FeedbackHOColumns(), [])
  ...
  return (
    <PageContainer>
      <div className="mb-8">
        ...
        <p className="text-muted-foreground">
          {selectedDivisionName
            ? `Ini adalah ringkasan aktivitas pada unit ${selectedDivisionName}`
            : 'Ini adalah ringkasan aktivitas pada seluruh Divisi Head Office PT. BMU.'}
        </p>
      </div>
      ...
      <div className="mt-8 space-y-4">
        ...
        <FeedbackHOTable
          table={table}
          isLoading={!dashboardData && divisionOptions.length === 0}
          isFetching={false}
          error={undefined}
          exportOptions={{ enabled: true, fileName: 'dashboard-feedbacks-headoffice', formats: ['csv', 'xlsx'], includeHeaders: true }}
        />
